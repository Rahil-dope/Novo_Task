import { $, $$, formatForInput, priorityRank, uid } from './utils.js';
import {
  ...
  createTaskCard, exportTasks, importTasks, getAnalytics
} from './taskManager.js';
...
let currentSubtasks = []; // Temporary storage for modal
...
function renderBoard() {
  ...
  Object.keys(columns).forEach(status => {
    ...
    const countEl = $(`.column[data-status="${status}"] h4`);
    if (countEl) countEl.dataset.count = columns[status].length;

    if (container) {
      // Optimization: Use DocumentFragment
      const fragment = document.createDocumentFragment();
      columns[status].forEach(t => {
        fragment.appendChild(createTaskCard(t));
      });
  ...
  if (completionChart) {
    ...
    completionChart.innerHTML = `
      <svg viewBox="0 0 200 200" style="width:100%;height:100%">
        <circle cx="100" cy="100" r="${radius}" fill="none" stroke="rgba(255,255,255,0.1)" stroke-width="20" />
        <circle cx="100" cy="100" r="${radius}" fill="none" stroke="url(#g1)" stroke-width="20" 
                stroke-dasharray="${circumference}" stroke-dashoffset="${offset}" stroke-linecap="round"
                transform="rotate(-90 100 100)" />
        <text x="100" y="100" fill="white" font-size="24" text-anchor="middle" dy="8">${data.completionRate}%</text>
        <text x="100" y="130" fill="rgba(255,255,255,0.6)" font-size="12" text-anchor="middle">Completion Rate</text>
      </svg>
    `;
  }

  // Productivity Chart (Bar)
  const productivityChart = $('#productivityChart');
  if (productivityChart) {
    const max = Math.max(...data.productivity.map(d => d.count), 5); // Min height of 5 for scale
    const bars = data.productivity.map((d, i) => {
      const height = (d.count / max) * 150;
      const x = i * 40 + 20;
      return `
        <rect x="${x}" y="${180 - height}" width="20" height="${height}" fill="rgba(96, 165, 250, 0.6)" rx="4" />
        <text x="${x + 10}" y="195" fill="rgba(255,255,255,0.6)" font-size="10" text-anchor="middle">${d.day}</text>
        <text x="${x + 10}" y="${180 - height - 5}" fill="white" font-size="10" text-anchor="middle">${d.count}</text>
      `;
    }).join('');
...
function renderProgress() {
  ...
  node.innerHTML = `
    <svg viewBox="0 0 36 36" style="width:100%;height:100%">
      ...
      <text x="18" y="20.5" fill="white" font-size="8" text-anchor="middle" font-weight="bold">${pct}%</text>
    </svg>
  `;
}
  ...
  try {
    ...
    if (addSubtaskBtn) {
      addSubtaskBtn.addEventListener('click', () => {
        ...
        if (title) {
          currentSubtasks.push({ id: uid(), title, completed: false });
          input.value = '';
...
function renderSubtasksInModal() {
  ...
  container.innerHTML = currentSubtasks.map((st, index) => `
    <div style="display:flex;align-items:center;gap:8px;background:rgba(255,255,255,0.05);padding:8px;border-radius:8px;">
      <input type="checkbox" ${st.completed ? 'checked' : ''} onchange="window.toggleSubtask(${index})" />
      <span style="flex:1;text-decoration:${st.completed ? 'line-through' : 'none'};color:${st.completed ? 'var(--text-muted)' : 'inherit'}">${escapeHtml(st.title)}</span>
      <button class="icon-btn" onclick="window.removeSubtask(${index})" style="padding:4px;font-size:12px">✕</button>
    </div>
  `).join('');
}
...
setTimeout(() => {
  ...
  if (tasks.length === 0) {
    addTask({
      ...
      subtasks: [
        { id: uid(), title: 'Create slides', completed: true },
        { id: uid(), title: 'Prepare demo', completed: false }
      ]
    });
    addTask({
